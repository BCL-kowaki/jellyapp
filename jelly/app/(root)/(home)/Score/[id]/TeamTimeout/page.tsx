  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const chartData = quarters.map(quarter => {
        ...
        return {
          quarter,
          teamA: Math.min(quarterData.filter(score => score.teamId === gameData.teamAId).reduce((sum, score) => sum + score.point, 0), 3),
          teamB: Math.min(quarterData.filter(score => score.teamId === gameData.teamBId).reduce((sum, score) => sum + score.point, 0), 3),
        }
      })
